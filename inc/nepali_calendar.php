	...
	class Nepali_Calendar {
		...
		public function eng_to_nep($yy,$mm,$dd){
			...
			if ($this->is_range_eng($yy, $mm, $dd) == false){
				...
				$this->nep_date["year"] = getNepaliNumerals($y);
				$this->nep_date["month"] = $m;
				$this->nep_date["date"] = getNepaliNumerals($total_nDays);
		...
		function to_nepali_time ($time) {
            $exptime = explode(':', $time);
            $neptime = '';
            $j = count($exptime);

            for ($i=0; $i < $j; $i++) { 
                $neptim = getNepaliNumerals($exptime[$i]);
                $neptime.=$neptim.":";
            }
        return rtrim($neptime,":");
        }			