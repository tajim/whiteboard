...
function clear_advert_main_cache() {
	if (function_exists('w3tc_pgcache_flush')) {
        w3tc_pgcache_flush();
    } 
}
add_action('acf/save_post', 'clear_advert_main_cache', 15);


/*
function mte_add_incontent_ad($content, $post)
{	if( is_single() && in_category( array(14,44,43,47,45,48,46), $post->ID ) ){
		$content_block = explode('<p>',$content);
		if(!empty($content_block[3]))
		{	$content_block[3] .= '<center><img src="https://setopati.com/wp-content/uploads/2017/08/m-nepal.jpg"></center>';
		}
